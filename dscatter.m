function [hAxes,col] = dscatter(X,Y, varargin)
...
green = [73 180 199]/255;
purple = [158 83 107]/255;
lambda = [];
nbins = [];
plottype = 'scatter';
contourFlag = false;
...
elseif length(k)>1
    ...
    switch(k)
        ...
        case 3 %'contour'
            [dummy, h] =contourf(ctrs1,ctrs2,F);
        case 4 %'image'
            nc = 256;
            F = F./max(F(:));
        ...
        case 5 %'scatter'
            ...
            col = F(ind);
            %col = repmat(col(:),1,3).*repmat(purple,length(col),1)+repmat(1-col(:),1,3).*repmat(green,length(col),1);
            if filled
